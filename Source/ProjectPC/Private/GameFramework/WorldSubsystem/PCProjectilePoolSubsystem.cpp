...
APCBaseProjectile* UPCProjectilePoolSubsystem::SpawnProjectile(const FTransform& SpawnTransform, FGameplayTag UnitTag, FGameplayTag TypeTag, const AActor* SpawnActor, const AActor* TargetActor, bool IsPlayerAttack)
{
	if (GetWorld() && GetWorld()->GetNetMode() != NM_Client)
	{
		if (SpawnActor && TargetActor)
		{
			...
			{
				SpawnedProjectile->ActiveProjectile(SpawnTransform, UnitTag, TypeTag, SpawnActor, TargetActor, IsPlayerAttack);
